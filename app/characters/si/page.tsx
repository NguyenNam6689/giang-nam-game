...
export default function SiCharactersPage() {
  const [characters, setCharacters] = useState<SiCharacter[]>([])
  const [loading, setLoading] = useState(true)
  const [submitting, setSubmitting] = useState(false)
  const [isAddDialogOpen, setIsAddDialogOpen] = useState(false)
  const [editingCharacter, setEditingCharacter] = useState<SiCharacter | null>(null)
  const [formData, setFormData] = useState<Partial<SiCharacter>>({})
  ...
  const loadCharacters = async () => {
    try {
      setLoading(true)
      const data = await getAllSiCharacters()
      setCharacters(data)
    } catch (error) {
      toast({
        title: "Lỗi",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingCharacter && editingCharacter.id) {
        ...
        await updateSiCharacter(editingCharacter.id, formData)
        ...
        // Add new character
        const newCharacter = await addSiCharacter({
          name: formData.name || "",
  ...
  const handleEdit = (character: SiCharacter) => {
    setEditingCharacter(character)
    setFormData(character)
    setIsAddDialogOpen(true)
  }

  const handleDelete = async (character: SiCharacter) => {
    ...
    try {
      await deleteSiCharacter(character.id)
      ...
      <div className="relative z-10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
            <div>
              <h1 className="text-4xl font-bold text-gray-900 mb-4 calligraphy">Nhân Vật Hệ Sĩ</h1>
              <p className="text-lg text-gray-700">Quản lý thông tin chi tiết các nhân vật hệ Sĩ</p>
            </div>
                  ...
                  <CardContent className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="bg-green-50 rounded p-3 text-center">
                        ...
                        <div className="font-semibold text-green-600">{character.construction}</div>
                      </div>
                      <div className="bg-green-50 rounded p-3 text-center">
                    ...
                    <div className="flex gap-2 mt-4">
                      <Button asChild className="flex-1 bg-blue-600 hover:bg-blue-700 text-white brush-cursor">
                        <Link href={`/characters/si/${character.id}`}>Xem Chi Tiết</Link>
                      </Button>
                    </div>
                  </CardContent>
                )}
